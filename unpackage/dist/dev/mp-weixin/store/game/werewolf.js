...
class WerewolfGame extends store_game_base.Game {
  constructor(config = {}) {
    super(config);
  ...
  generateDefaultRoles() {
    const rolePool = utils_werewolfConfig.getRolePoolByPlayerCount(this.playerCount);
    const pool = [];
    rolePool.forEach((roleId) => {
      const roleInfo = utils_werewolfConfig.roleConfig[roleId];
      if (roleInfo) {
        pool.push({
          id: roleId,
          name: roleInfo.name,
...
//# sourceMappingURL=../../../.sourcemap/mp-weixin/store/game/werewolf.js.map
