...
// 根据玩家人数获取身份配置
export function getRolePoolByPlayerCount(playerCount) {
  // 向下取整处理
  let adjustedCount = playerCount
  if (adjustedCount > 12) adjustedCount = 12
  if (adjustedCount < 6) adjustedCount = 6
  
  // 特殊处理中间值
  if (adjustedCount === 7) adjustedCount = 6
  if (adjustedCount === 11) adjustedCount = 10
  
  switch (adjustedCount) {
    case 6:
      return [
        'werewolf', 'werewolf',       // 2狼人
        'seer', 'witch',             // 2神职
    ...
    case 10:
      return [
        'werewolf', 'werewolf', 'werewolf', // 3狼人
        'seer', 'witch', 'hunter', 'guard', // 4神职
        'villager', 'villager', 'villager' // 3平民
      ]
    case 12:
      return [
        'whiteWolfKing', 'wolfBeauty', 'werewolf', // 3狼人（含白狼王和狼美人）
        'seer', 'witch', 'hunter', 'guard', // 4神职
...
// 获取阵营图标
export function getCampIcon(camp) {
  switch (camp) {
    case 'wolf':
      return '🐺'
    case 'god':
